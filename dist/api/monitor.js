...
router.get('/jobs', async (req, res) => {
    try {
        const sector = req.query.sector; // academia|industry|international|india
        const region = req.query.region; // asia|europe|north_america
        const subsector = req.query.subsector; // industry  (india sector only)
       ...
       OFFSET $${params.length}`, params);
        ...
        if (result.rows.length === 0) {
            ...
            if (region === 'asia')
                liveOrgNames = LIVE.asia;
            else if (region === 'europe')
                liveOrgNames = LIVE.europe;
            else if (region === 'north_america')
                liveOrgNames = LIVE.north_america;
            else if (sector === 'india' && subsector === 'industry')
                liveOrgNames = LIVE.india_industry;
            else if (sector === 'india')
                liveOrgNames = [...LIVE.india_academic, ...LIVE.india_industry];
            else if (sector === 'industry')
                liveOrgNames = [...LIVE.asia, ...LIVE.europe, ...LIVE.north_america];
            else if (sector === 'international')
                liveOrgNames = LIVE.international;
        }
            ...
            const EUROPE_C = new Set(['UK', 'Germany', 'Switzerland', 'France', 'Denmark',
                'Ireland', 'Netherlands', 'Belgium', 'Sweden']);
            if (!result.rows.some((r) => ASIA_C.has(r.country)))
                liveOrgNames.push(...LIVE.asia);
            if (!result.rows.some((r) => EUROPE_C.has(r.country)))
                liveOrgNames.push(...LIVE.europe);
        }
        // ── Run live search if needed ────────────────────────────────────────────
        if (liveOrgNames.length > 0) {
            const nameSet = new Set(liveOrgNames);
            const liveOrgs = orgConfig_1.MONITOR_ORGS.filter(o => o.apiType === 'websearch' && nameSet.has(o.name));
            try {
                const settled = await Promise.allSettled(liveOrgs.map(org => (0, monitorEngine_1.scanViaWebSearch)(org)));
                const liveJobs = [];
                const existingKeys = new Set(result.rows.map((r) => `${r.title}|${r.org_name}`));
                settled.forEach((r, i) => {
                ...
                console.error('[Monitor] Live fallback error:', fallbackErr.message);
            }
        }
