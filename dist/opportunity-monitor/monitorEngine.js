...
async function scanViaWebSearch(org) {
    ...
    try {
        ...
        }), 10000, `Serper search for ${org.name}`);
        if (!response.ok) {
        ...
        console.error(`[Monitor] Serper failed for ${org.name}:`, err.message);
        return [];
    }
}
    ...
    try {
        ...
        for (const item of allItems.slice(0, 20)) {
            const title = (item.match(/<title><!\[CDATA\[(.*?)\]\]><\/title>/)?.[1] ||
                item.match(/<title[^>]*>(.*?)<\/title>/)?.[1] || '').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').trim();
            ...
            item.match(/<description><!\[CDATA\[([\s\S]*?)\]\]><\/description>/)?.[1] ||
                ...
                item.match(/<content[^>]*>([\s\S]*?)<\/content>/)?.[1] || '').replace(/<[^>]+>/g, '').slice(0, 150).trim();
        ...
        console.error(`[RSS] ${org.name}: failed — ${err.message}`);
        return [];
    }
}
    ...
    if ((org.apiType === 'rss' || org.apiType === 'usajobs') && jobs.length > 0) {
        try {
         ...
         WHERE org_id = $1
           AND is_active = true
           AND last_seen_at < $2
         RETURNING id`, [orgId, scanStart]);
            if (expired.rows.length > 0) {
                console.log(`[Monitor] ${org.name}: expired ${expired.rows.length} stale listing(s)`);
            }
        }
        catch (err) {
            console.error('[Monitor] Stale-job cleanup failed:', err.message);
        }
    }
    try {
