...
function isDirectJobUrl(url: string): boolean {
  ...
  if (lower.includes('glassdoor.com/job') || lower.includes('glassdoor.co.in/job')) return true
  // Accept Indeed job pages
  if (lower.includes('indeed.com') && (lower.includes('/job') || lower.includes('viewjob'))) return true
  ...
  if (lower.includes('reed.co.uk') || lower.includes('totaljobs.com') ||
      ...
      lower.includes('michaelpage.co.uk') || lower.includes('robertwalters.co.uk')) return true
...
async function scanViaAdzuna(org: DJMonitorOrg): Promise<DJScannedJob[]> {
  ...
  try {
    ...
    console.error(`[MonitorDJ-Adzuna] Failed ${org.name}:`, (err as Error).message)
    return []
  }
}
...
export async function scanOrgDJ(orgId: string, org: DJMonitorOrg): Promise<{
  ...
  try {
    switch (org.apiType) {
      ...
      case 'adzuna':   jobs = await scanViaAdzuna(org);     break
