...
export function filterAndScoreJobs(
  ...
  if (candidate.id === 'pooja' && candidate.track) {
    const trackData = poojaProfiles[candidate.track];
    if (trackData) {
      scoringProfile = {
        ...candidate,
  ...
  return jobs
    ...
    .filter(job => {
      ...
      if (candidate.id === 'pooja' && candidate.track) {
        const jobTrack = classifyAcademicIndustry(job);
        if (jobTrack !== candidate.track) return false;
      }
    ...
    // ── Score ──────────────────────────────────────────────────────────────
    .map(job => ({ ...job, matchScore: computeMatchScore(job, scoringProfile) }))
