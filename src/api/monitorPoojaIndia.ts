  ...
  try {
    ...
    const noiseDel = await client.query(`
      DELETE FROM pooja_india_monitor_jobs WHERE
        ...
        -- Additional title noise
        OR LOWER(title) LIKE '%tiger cell%'
        OR LOWER(title) LIKE '%genomic facility%'
        OR LOWER(title) LIKE '%imaging facility%'
        OR LOWER(title) LIKE '%flow cytometry%'
        OR LOWER(title) LIKE '%gallery%'
        OR LOWER(title) LIKE '%notification for appointment of two members%'
        OR LOWER(title) LIKE '%cell processing system%'
        OR LOWER(title) LIKE '%about recruitment | %'
        ...
        OR LOWER(apply_url) LIKE '%/staff/%' OR LOWER(apply_url) LIKE '%/abouts/staff%'
        ...
        OR LOWER(apply_url) LIKE '%/home/nams%'
        ...
        OR LOWER(apply_url) LIKE '%/research-facilities%'
        ...
        OR LOWER(apply_url) LIKE '%examination/results%'
        OR LOWER(apply_url) LIKE '%results_final_%'
        ...
        OR LOWER(apply_url) LIKE '%/admissions?%'
        OR LOWER(apply_url) LIKE '%/laboratories%'
        OR LOWER(apply_url) LIKE '%/about-recruitment%'
        OR LOWER(apply_url) LIKE '%/notifications%'
        OR LOWER(apply_url) LIKE '%/latest/%'
        OR LOWER(apply_url) LIKE '%/scientist-profile%'
        OR LOWER(apply_url) LIKE '%/imagegallery%'
        OR LOWER(apply_url) LIKE '%/our_team%'
        OR LOWER(apply_url) LIKE '%/our-team%'
        OR LOWER(apply_url) LIKE '%/tiger-cell%'
        OR LOWER(apply_url) LIKE '%/genomic-facility%'
        OR LOWER(apply_url) LIKE '%/tag/%'
        OR LOWER(apply_url) LIKE '%/en/notices/81-%'
        OR LOWER(apply_url) LIKE '%/index.php/en/node%'
        OR LOWER(apply_url) LIKE '%/nipgrv4/latest%'
    `)
  ...
  try {
    const result = await pool.query(`
      DELETE FROM pooja_india_monitor_jobs WHERE
        ...
        OR LOWER(title) LIKE '%on contract%' OR LOWER(title) LIKE '%contractual basis%'
        OR LOWER(title) LIKE '%fixed term%' OR LOWER(title) LIKE '%externally funded%'
        -- Additional title noise (non-job pages)
        OR LOWER(title) LIKE '%tiger cell%'
        OR LOWER(title) LIKE '%genomic facility%'
        OR LOWER(title) LIKE '%imaging facility%'
        OR LOWER(title) LIKE '%flow cytometry%'
        OR LOWER(title) LIKE '%nams -%' OR LOWER(title) LIKE '%| nams%'
        OR LOWER(title) LIKE '%gallery%'
        OR LOWER(title) LIKE '%notification for appointment of two members%'
        OR LOWER(title) LIKE '%cell processing system%'
        OR LOWER(title) LIKE '%about recruitment | %'
        -- Person names without Dr./Prof. prefix (tag/profile pages)
        OR LOWER(apply_url) LIKE '%/tag/%'
        OR LOWER(apply_url) LIKE '%/en/notices/81-%'
        ...
        OR LOWER(apply_url) LIKE '%/abouts/staff%'
        OR LOWER(apply_url) LIKE '%/rd-staff%'
