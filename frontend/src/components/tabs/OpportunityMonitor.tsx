...
const INDIA_ORG_SUBSECTOR: Record<string, IndiaSubsector> = {
  'IISc Bangalore': 'Academic',
  'IISER Pune': 'Academic',
  'IIT Bombay Biology': 'Academic',
  'IIT Bombay Biosciences': 'Academic',
  'IIT Delhi Biochemical Engineering': 'Academic',
  'JNU School of Life Sciences': 'Academic',
  'IISER Kolkata': 'Academic',
  'IISER Bhopal': 'Academic',
  'IISER Mohali': 'Academic',
  'IISER Thiruvananthapuram': 'Academic',
  'ICMR HQ': 'Govt Research',
  'CSIR-IGIB Delhi': 'Govt Research',
  'DBT-THSTI Faridabad': 'Govt Research',
  'AIIMS New Delhi': 'Govt Research',
  'JNCASR Bangalore': 'Govt Research',
  'NCBS Bangalore': 'Govt Research',
  'TIFR Mumbai': 'Govt Research',
  'CCMB Hyderabad': 'Govt Research',
  'IGIB Delhi': 'Govt Research',
  'inStem Bangalore': 'Govt Research',
  'NII Delhi': 'Govt Research',
  'RCB Faridabad': 'Govt Research',
  'RGCB Trivandrum': 'Govt Research',
  'IndiaBioscience Jobs': 'Govt Research',
  'Employment News Research': 'Govt Research',
  'Biocon Biologics': 'Industry',
  'Biocon Research': 'Industry',
  'Syngene International': 'Industry',
  'AstraZeneca India': 'Industry',
}
...
export function OpportunityMonitor() {
  ...
  const filteredJobs = jobs
    ...
    .filter(j => activeSector !== 'india' || indiaSubsector === 'All' ||
      (INDIA_ORG_SUBSECTOR[j.org_name] ?? 'Govt Research') === indiaSubsector)
    .filter(j =>
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.tabs}>
        {(Object.keys(SECTOR_CONFIG) as Sector[]).map(s => {
          ...
          return (
            <button 
              key={s} 
              onClick={() => { setActiveSector(s); setSelectedOrg(null); setIndiaSubsector('All'); }}
              style={{
                ...styles.tab,
      ...
      {activeSector === 'india' && (
        <div style={styles.indiaSubsectorRow}>
          {INDIA_SUBSECTORS.map(sub => (
            <button
              key={sub}
              onClick={() => setIndiaSubsector(sub)}
              style={{
                ...styles.subsectorBtn,
                background: indiaSubsector === sub ? 'rgba(236,72,153,0.15)' : 'transparent',
                border: `1px solid ${indiaSubsector === sub ? '#ec4899' : 'rgba(255,255,255,0.1)'}`,
                color: indiaSubsector === sub ? '#ec4899' : 'var(--text-secondary)',
                fontWeight: indiaSubsector === sub ? 800 : 500,
              }}
            >
              {sub}
            </button>
            ...
            <div key={job.id} className="glass" style={{
              ...
              <div style={styles.jobMain}>
                <div style={styles.jobHeader}>
                  ...
                  {isHighSignal(job) && (
                    <span style={styles.highSignalBadge}>⚡ High Signal</span>
                  )}
                  {getRoleBadge(job.title) && (
                    <span style={{ ...styles.roleBadge, ...getRoleBadgeStyle(job.title) }}>
                      {getRoleBadge(job.title)}
                    </span>
                  )}
                </div>
              ...
              <div style={styles.emptyCaughtUp}>
                {activeSector === 'india' ? (
                  <>
                    ...
                    <p style={{ fontWeight: 700 }}>Indian institute scan runs daily.</p>
                    <p style={{ fontSize: 13, opacity: 0.7 }}>
                      First results appear after next scheduled scan (08:00 UTC).<br />
                      Click <strong>Scan Now</strong> to fetch immediately.
                    </p>
                  </>
                ) : (
                  <>
...
// ── Badge helpers ─────────────────────────────────────────────────────────────
const HIGH_SIGNAL_TITLE_TERMS = ['cardiovascular', 'cardiac', 'molecular biology', 'heme', 'cardiomyopathy', 'ptrh2']

function isHighSignal(job: MonitorJob): boolean {
  if ((job.relevanceScore ?? 0) >= 3) return true
  const t = job.title.toLowerCase()
  const s = (job.snippet || '').toLowerCase()
  return HIGH_SIGNAL_TITLE_TERMS.some(kw => t.includes(kw)) ||
    s.includes('cardiovascular') || s.includes('genetics')
}

function getRoleBadge(title: string): string | null {
  const t = title.toLowerCase()
  if (t.includes('postdoctoral') || t.includes('postdoc')) return 'Postdoc'
  if (t.includes('assistant professor') || t.includes('faculty')) return 'Faculty'
  if (t.includes('research associate')) return 'RA'
  if (t.includes('scientist')) return 'Scientist'
  return null
}

function getRoleBadgeStyle(title: string): React.CSSProperties {
  const t = title.toLowerCase()
  if (t.includes('postdoctoral') || t.includes('postdoc'))
    return { background: 'rgba(59,130,246,0.15)', color: '#60a5fa', borderColor: 'rgba(59,130,246,0.3)' }
  if (t.includes('assistant professor') || t.includes('faculty'))
    return { background: 'rgba(168,85,247,0.15)', color: '#c084fc', borderColor: 'rgba(168,85,247,0.3)' }
  if (t.includes('research associate'))
    return { background: 'rgba(148,163,184,0.1)', color: '#94a3b8', borderColor: 'rgba(148,163,184,0.2)' }
  // scientist
  return { background: 'rgba(20,184,166,0.15)', color: '#2dd4bf', borderColor: 'rgba(20,184,166,0.3)' }
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  // India sub-sector filter row
  indiaSubsectorRow: { display: 'flex', gap: 8, padding: '12px 0 4px' },
  subsectorBtn: { padding: '5px 14px', borderRadius: 6, cursor: 'pointer', fontSize: 12, transition: 'all 0.15s' },

  // High Signal badge
  highSignalBadge: { fontSize: 9, fontWeight: 900, padding: '2px 7px',
    background: 'rgba(245,158,11,0.15)', color: '#fbbf24',
    border: '1px solid rgba(245,158,11,0.35)', borderRadius: 4, flexShrink: 0 },

  // Role type badge base (colour overridden per-role by getRoleBadgeStyle)
  roleBadge: { fontSize: 9, fontWeight: 800, padding: '2px 7px',
